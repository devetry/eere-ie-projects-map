...
(function () {
  ...
  function linkTitle(dataarray) {
    dataarray.forEach(function (row) {
      row.Project = "<a target=\"_blank\" href=\"".concat(row.Link, "\">").concat(row.Project, "</a>");
    });
  }
  /**
  ...
  function renderMap(cfg) {
    ...
    spiderifier = new MapboxglSpiderifier(map, {
      customPin: true,
      initializeLeg: function initializeLeg(spiderLeg) {
        var $spiderPinCustom = $("<div>", {
          "class": "spider-point-circle"
        });
        ...
        var content = "<h4>" + marker.Tribe + "</h4>";
  ...
  function updateSource(dataarray) {
    ...
    geojsondata.features = geojsondata.features.filter(function (feat) {
  ...
  function renderMapMarkers(dataarray) {
    ...
    map.on("load", function () {
      ...
      map.addLayer({
        ...
        paint: {
          "circle-color": ["step", ["get", "point_count"], "rgba(110, 204, 57, 0.6)", 10, "rgba(240, 194, 12, 0.6)", 50, "rgba(241, 128, 23, 0.6)"],
          "circle-radius": 20
        }
      });
      map.addLayer({
      ...
      map.on("click", "clusters", function (e) {
        ...
        if (!features.length || map.getZoom() !== 13) {
          ...
          map.getSource("locations").getClusterLeaves(clusterId, 100, 0, function (err, leafFeatures) {
            if (err) {
      ...
      map.on("click", "unclustered-point", function (e) {
        ...
        var content = "<h4>" + e.features[0].properties.Tribe + "</h4>";
  ...
  function countMarkersFromState(state, dataarray) {
    ...
    $("#count-alaska").text(count); //console.log('found '+ arr.length + ' markers from ' + state)
  }
  /**
  ...
  function filterDataTable() {
    Object.keys(config.uiFilters).forEach(function (filter) {
      var idx = config.dataHeaders.indexOf(filter); // build out regex terms that match the word/phrase exactly
